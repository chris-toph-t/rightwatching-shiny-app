...
################################ Start Population Density from Kontur ###############
    
    if (file.exists("../data/pop.gpkg/")) {
      curl::curl_download(url = "https://geodata-eu-central-1-kontur-public.s3.amazonaws.com/kontur_datasets/kontur_population_DE_20220630.gpkg.gz", destfile = "../data/pop.gpkg.gz")  
      # incase admin boundaries population estimates needed. careful, this file is larger thouhg less granular due to complex geometries.
      #curl::curl_download(url = "https://geodata-eu-central-1-kontur-public.s3.eu-central-1.amazonaws.com/kontur_datasets/kontur_boundaries_DE_20220407.gpkg.gz", destfile = "../data/pop.gpkg.gz")
      
      R.utils::gunzip("../data/pop.gpkg.gz")
    }
    
    popkontur <- read_sf("../data/pop.gpkg") 
    popkontur_filtered <- st_intersection(st_transform(kreise, crs = 3857), popkontur) %>% 
        st_transform(crs = 4326)
    
    rm(popkontur)
################################ End Population Density from Kontur ###############
...
#################################Start nationality stats data from regionalstatistik##########################
    nationality_data <- dg_call(nuts_nr = "3", stat_name = "BEVSTD", substat_name = "NAT",parameter = c("NATA", "NATD"), year = "2020", parent_chr = bundesland)
    ...
    votes_data <- dg_call(nuts_nr = "3", stat_name = "WAHL09", substat_name = "PART04", year = "2021", parent_chr = bundesland)
    voters_data <- dg_call(nuts_nr = "3", stat_name = "WAHL01", year = "2021", parent_chr = bundesland)
