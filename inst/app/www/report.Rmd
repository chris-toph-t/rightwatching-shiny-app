...
`r paste0("Vorfälle in der Chronik gefunden: ", nrow(chronik_enriched), ". <br>Diese Analyse nutzt ", nrow(chronik_filtered()), "<br>", "Davon auf Karte lokalisiert: ", nrow(filter(chronik_filtered(), !is.na(place)))," d.h. ", nrow(filter(chronik_filtered(), is.na(place))), " nicht lokalisiert.<br>", "Vom ", format.Date(min(chronik_filtered()$date), "%d.%m.%Y"), " bis ", format.Date(max(chronik_filtered()$date), "%d.%m.%Y"), ".")`
...
knitr::kable(select(chronik_missing, descr_text, date, admin6, place, title, latitude, longitude, source_name))
```
